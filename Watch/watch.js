...
export default class Watch extends THREE.Group {
    constructor(cityName, center = new THREE.Vector2(0.0, 0.0), radius = 0.75, nameBackgroundColor = 0xffffff, nameForegroundColor = 0x000000, dialColor = 0x000000, markersColor = 0xffffff, handsHMColor = 0xffffff, handSColor = 0xff0000) {
        ...
        console.log("Watch: " + this.cities[this.cityIndex].name + " (" + this.cities[this.cityIndex].timeZone + ")");
        ...
        const hourArc = 6.0;
        const minuteArc = 30.0;

        /* Hour Lines */
        for (let i = 0; i < 12; i++) {
            points.push(new THREE.Vector2(radius0 * Math.cos(i * Math.PI / hourArc), radius0 * Math.sin(i * Math.PI / hourArc)));
            points.push(new THREE.Vector2(radius2 * Math.cos(i * Math.PI / hourArc), radius2 * Math.sin(i * Math.PI / hourArc)));
        }
        ...
        for (let i = 0; i < 60; i++) {
            ...
            points.push(new THREE.Vector2(radius1 * Math.cos(i * Math.PI / minuteArc), radius1 * Math.sin(i * Math.PI / minuteArc)));
            points.push(new THREE.Vector2(radius2 * Math.cos(i * Math.PI / minuteArc), radius2 * Math.sin(i * Math.PI / minuteArc)));
        }

        /* ...
        geometry = new THREE.BufferGeometry()...;
        material = new THREE.LineBasicMaterial(...);
        this.markers = new THREE.LineSegments(...);
        ...
        let hourHand = [];
        hourHand.push(new THREE.Vector2(0.0, 0.0));
        hourHand.push(new THREE.Vector2(0.5 * radius, 0.0));

        geometry = new THREE.BufferGeometry().setFromPoints(hourHand);
        ...
        let minuteHand = [];
        minuteHand.push(new THREE.Vector2(0.0, 0.0));
        minuteHand.push(new THREE.Vector2(0.7 * radius, 0.0));

        geometry = new THREE.BufferGeometry().setFromPoints(minuteHand);
        material = new THREE.LineBasicMaterial({ color: handsHMColor });
    ...
    update() {
        ...
        let hourAngle = Math.PI / 2.0 - 2.0 * Math.PI * minuteAngle / 12.0 + time[0];
