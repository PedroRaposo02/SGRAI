...
export default class Table extends THREE.Group {
    constructor(parameters) {
        ...
        // Create the net (a dashed line)
        const endpoints = [
            new THREE.Vector2(0.0, -this.halfSize.y),
            new THREE.Vector2(0.0, this.halfSize.y),
        ];
        const dashSize = this.size.y / (2.0 * this.dashes - 1.0);
        const gapSize = dashSize;

        geometry = new THREE.BufferGeometry().setFromPoints(endpoints);
        material = new THREE.LineDashedMaterial({ color: this.color, dashSize: dashSize, gapSize: gapSize });
